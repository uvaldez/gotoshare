...
function initRTCCopy() {
	...
	/*$(â€œ#encryption_type").change(function() {
...
function process_room_state(data) {
	if (data.browser != "") { /* will be blank if new room */
		...
		if (data.encryption == "NONE") {
			$("#room_state").append('This room already exists and the creator used:<br /> <span style="color:'+browser_color+'">'+ sanitize(data.browser) + '</span> <span style="color:'+browser_color+'">' + sanitize(data.browserVer) + '</span><br /><br />');
		} else {
			$("#room_state").append('This room already exists and the creator used:<br /> <span style="color:'+browser_color+'">'+ sanitize(data.browser) + '</span> <span style="color:'+browser_color+'">' + sanitize(data.browserVer) + '</span><br /><br />');
		}
		
		/* set the dropdown box to default to the encryption value */
		/*$(â€œ#encryption_type").val(data.encryption);
		if (document.getElementById("encryption_type").value != "NONE") {
			$("#encryption_note").show();
		}*/
...
function transition_from_username_to_main() {
	...
	encryption_type = "NONE";//document.getElementById("encryption_type").value;
